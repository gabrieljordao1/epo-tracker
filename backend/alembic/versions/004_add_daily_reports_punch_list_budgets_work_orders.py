...
def upgrade() -> None:
    # If all tables exist, skip entirely (idempotent)
    if (_table_exists('daily_reports') and _table_exists('punch_items')
            and _table_exists('community_budgets') and _table_exists('work_orders')):
        return

    # ─── Daily Reports ────────────────────────────────────────
    if not _table_exists('daily_reports'):
        op.create_table(
            'daily_reports',
    ...
    # ─── Punch Items ──────────────────────────────────────────
    if not _table_exists('punch_items'):
        op.create_table(
            'punch_items',
            sa.Column('id', sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column('company_id', sa.Integer(), sa.ForeignKey('companies.id'), nullable=False, index=True),
    ...
    # ─── Community Budgets ────────────────────────────────────
    if not _table_exists('community_budgets'):
        op.create_table(
            'community_budgets',
            sa.Column('id', sa.Integer(), primary_key=True, autoincrement=True),
            sa.Column('company_id', sa.Integer(), sa.ForeignKey('companies.id'), nullable=False, index=True),
